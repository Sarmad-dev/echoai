...
interface AuthContextType {
  ...
  signInWithProvider: (
    provider: SignInWithOAuthCredentials
  ) => Promise<{ error: AuthError | null }>;
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    try {
      const {
        ...
        if (session?.user) {
          console.log(
            "🔄 Fetching user profile after auth change:",
            session.user.id
          );
          const profile = await fetchUserProfile(session.user.id);
          if (profile) {
            console.log(
              "✅ User profile loaded after auth change:",
              profile.role
            );
            setUserProfile(profile);
          } else {
      ...
      setError(
        error instanceof Error
          ? error.message
          : "Failed to setup authentication listener"
      );
      setLoading(false);
    }
  }, [supabase]);
  ...
  const signUp = async (email: string, password: string) => {
    try {
      ...
      return {
        error: { message: "Failed to sign up. Please try again." } as AuthError,
      };
    }
  };

  const signInWithProvider = async (provider: SignInWithOAuthCredentials) => {
    try {
      ...
      return {
        error: { message: "Failed to sign in. Please try again." } as AuthError,
      };
    }
  };
